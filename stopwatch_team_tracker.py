...
def get_round_roster(cursor, round_date: str, round_num: int, team: int) -> Set[str]:
    """
    ...
    cursor.execute("""
        WITH RankedStats AS (
            ...
            WHERE round_date = ? AND round_number = ? AND team = ?
        )
        SELECT DISTINCT player_guid, player_name
        FROM RankedStats
        WHERE rn = 1
    """, (round_date, round_num, team))
    
    return {row[0]: row[1] for row in cursor.fetchall()}

def detect_stopwatch_teams(round_date: str):
    """
    ...
    print("="*80)
    print(f"STOPWATCH TEAM DETECTION: {round_date}")
    print("="*80)
    ...
    r1_axis = get_round_roster(cursor, round_date, rounds[0], 1)
    r1_allies = get_round_roster(cursor, round_date, rounds[0], 2)
    ...
    for round_num in rounds:
        ...
        axis_players = get_round_roster(cursor, round_date, round_num, 1)
        allies_players = get_round_roster(cursor, round_date, round_num, 2)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python stopwatch_team_tracker.py <round_date>")
        print("Example: python stopwatch_team_tracker.py 2024-10-28")
        sys.exit(1)
    
    round_date = sys.argv[1]
    result = detect_stopwatch_teams(round_date)
