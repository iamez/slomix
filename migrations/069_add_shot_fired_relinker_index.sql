...
-- The mismatch leg reads round_id IS NOT NULL rows and joins rounds on it, so
-- it cannot use the partial index above. 068 left this shape out because the
-- mismatch legs there run over much smaller tables; at 775k rows the join
-- source needs its own access path.
