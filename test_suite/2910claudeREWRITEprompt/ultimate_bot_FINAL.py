...
class SeasonManager:
    ...
    def __init__(self):
        ...
        logger.info(f"📅 SeasonManager initialized - Current: {self.get_current_season()}")
...
class AchievementSystem:
    ...
    async def _send_achievement_notification(self, achievement, channel):
        ...
        try:
            ...
            logger.info(f"🏆 Achievement notification sent: {achievement['player_name']} - {ach_data['title']}")
...
class ETLegacyCommands(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        logger.info("✅ ETLegacyCommands initialized with caching, achievements & seasons")
        ...
        try:
            ...
            async with aiosqlite.connect(self.bot.db_path) as db:
                ...
                # Handle no arguments - use author's linked account
                    ...
                    if not link:
                        await ctx.send("❌ Please link your account with `!link` or specify a player name!")
                        return
                ...
                for threshold, ach in sorted(self.achievements.KILL_MILESTONES.items()):
                    if kills >= threshold:
                        kill_progress.append(f"✅ {ach['emoji']} **{ach['title']}** ({threshold:,} kills)")
                    else:
                        remaining = threshold - kills
                        kill_progress.append(f"🔒 {ach['emoji']} {ach['title']} - {remaining:,} kills away")
                ...
                for threshold, ach in sorted(self.achievements.GAME_MILESTONES.items()):
                    if games >= threshold:
                        game_progress.append(f"✅ {ach['emoji']} **{ach['title']}** ({threshold:,} games)")
                    else:
                        remaining = threshold - games
                        game_progress.append(f"🔒 {ach['emoji']} {ach['title']} - {remaining:,} games away")
                ...
                if games >= 20:
                    ...
                    for threshold, ach in sorted(self.achievements.KD_MILESTONES.items()):
                        if kd_ratio >= threshold:
                            kd_progress.append(f"✅ {ach['emoji']} **{ach['title']}** ({threshold:.1f} K/D)")
                        else:
                            needed = threshold - kd_ratio
                            kd_progress.append(f"🔒 {ach['emoji']} {ach['title']} - {needed:.2f} K/D away")
    ...
    async def compare(self, ctx, player1_name: str, player2_name: str):
        ...
        try:
            ...
            async with aiosqlite.connect(self.bot.db_path) as db:
                ...
                logger.info(f"📊 Comparison generated: {p1_stats['name']} vs {p2_stats['name']}")
    ...
    async def season_info(self, ctx):
        ...
        try:
            ...
            embed.set_footer(text="Use !leaderboard to see full rankings • Seasons reset quarterly")
    ...
    async def list_guids(self, ctx, *, search_term: str = None):
        """
        🔍 List unlinked player GUIDs for admin linking
        
        Shows unlinked players with their aliases to help admins link accounts.
        
        Usage:
        - !list_guids              → Top 10 most active unlinked players
        - !list_guids recent       → Recently active (last 7 days)
        - !list_guids <name>       → Search for specific player
        - !list_guids all          → Show all unlinked (max 20)
        """
        try:
            async with aiosqlite.connect(self.bot.db_path) as db:
                # Base query for unlinked GUIDs
                base_where = "WHERE pa.guid NOT IN (SELECT et_guid FROM player_links WHERE et_guid IS NOT NULL)"
                
                if search_term and search_term.lower() not in ('all', 'recent'):
                    ...
                    query = f"""
                        ...
                        {base_where}
                        AND LOWER(pa.alias) LIKE LOWER(?)
                        GROUP BY pa.guid
                        ORDER BY last_seen DESC, total_kills DESC
                        LIMIT 10
                    """
                    params = (f'%{search_term}%',)
                    title = f"🔍 Unlinked Players matching '{search_term}'"
                    
                elif search_term and search_term.lower() == 'recent':
                    # Recent mode - last 7 days
                    ...
                    """
                    params = ()
                    title = "📋 All Unlinked Players (Top 20)"
                    
                else:
                    # Default mode - top 10 by activity
                    query = f"""
                        SELECT 
                            pa.guid,
                            COUNT(DISTINCT pa.alias) as alias_count,
                    ...
                    """
                    params = ()
                    title = "🎮 Most Active Unlinked Players (Top 10)"
                ...
                embed = discord.Embed(
                    title=title,
                    description=(
                        f"Found **{len(results)}** unlinked player(s). "
                        f"Showing up to 2 aliases per GUID.\n\n"
                        f"💡 **To link**: `!link @user <GUID>`"
                    ),
                    color=0x3498db
                )
                
                # Process each GUID
                for guid, alias_count, last_seen, kills, deaths, games in results:
                    kills = kills or 0
                    deaths = deaths or 0
                    games = games or 0
                    kd = kills / deaths if deaths > 0 else kills
                    
                    # Get top 2 aliases for this GUID
                    async with db.execute("""
                        ...
                        LIMIT 2
                    """, (guid,)) as alias_cursor:
                        aliases = await alias_cursor.fetchall()
                    
                    # Format aliases
                    if aliases:
                        primary = aliases[0][0]
                        alias_text = f"**{primary}**"
                        if len(aliases) > 1:
                            alias_text += f" / {aliases[1][0]}"
                        if alias_count > 2:
                            alias_text += f" _(+{alias_count - 2} more)_"
                    else:
                        alias_text = "_No aliases found_"
                    ...
                    embed.add_field(
                        ...
                        value=(
                            f"{alias_text}\n"
                            f"📊 {kills:,}K / {deaths:,}D / {kd:.2f} KD\n"
                            f"🎮 {games:,} games • Last: {last_seen or 'Unknown'}"
                        ),
                        inline=False
                    )
                ...
                embed.set_footer(
                    text=(
                        f"💡 Use !list_guids <name> to search • "
                        f"!list_guids recent for last 7 days • "
            ...
            logger.error(f"Error in list_guids command: {e}", exc_info=True)
            await ctx.send(f"❌ Error listing GUIDs: {e}")
