...
This audit examined the ET:Legacy stats ingestion and parser pipeline (SSH → local_stats → parser → DB import → processed_files → Discord). **8 critical bugs were identified**, with 2 HIGH-PRIORITY bugs causing complete data loss and incorrect statistics. Fixes have been developed and tested for the critical bugs.
...
#### BUG #4: Player GUID Changes
**Severity**: MEDIUM - Data Quality  
**Impact**: Player reconnects between rounds cause differential to treat them as new

**Description**:
If a player disconnects and reconnects between Round 1 and Round 2, their GUID may change, causing the differential calculation to not find them in Round 1 data.

**Mitigation**:
- This is expected behavior for reconnects
- Could implement fuzzy matching on player name instead of GUID
...
Test #4: Player GUID Consistency   ✅ PASS
...
The ET:Legacy stats parser has **2 critical bugs causing data loss and incorrect statistics**. Both have been fixed and tested. The most urgent remaining issue is lack of database transactions (Bug #7), which could cause permanent data loss if writes fail partway through.
...
**Fixes Status**: 2/8 IMPLEMENTED, 6/8 DOCUMENTED
