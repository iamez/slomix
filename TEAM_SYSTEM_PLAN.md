...
- ✅ Automatic team detection from session data
- ✅ Handle Axis/Allies role switching (stopwatch mode)
- ✅ Identify persistent teams across all maps in a session
- ✅ Handle late joiners using co-membership voting
- ✅ Store teams in `session_teams` table

### 2. Team Names
- ✅ Default: "Team A" vs "Team B"
- 🔄 Custom names: Allow setting custom team names per session
...
-- Add indexes
CREATE INDEX idx_session_results_date ON session_results(session_date);
CREATE INDEX idx_map_results_date ON map_results(session_date);
...
2. **Last Session Command** (`bot/cogs/last_session_cog.py`)
...
3. Verify lineup change detection across multiple sessions
