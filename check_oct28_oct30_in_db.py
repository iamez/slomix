...
cursor.execute("SELECT COUNT(*) FROM sessions WHERE session_date LIKE '2025-10-28%'")
oct28_count = cursor.fetchone()[0]
print(f"Oct 28 sessions in DB: {oct28_count}")

# Check Oct 30 sessions
cursor.execute("SELECT COUNT(*) FROM sessions WHERE session_date LIKE '2025-10-30%'")
...
if oct28_count > 0 or oct30_count > 0:
    print("\n📅 Sample sessions:")
    cursor.execute("""
        SELECT session_date, map_name, id
        FROM sessions 
