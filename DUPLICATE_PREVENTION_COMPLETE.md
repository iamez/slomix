...
CREATE TABLE player_comprehensive_stats (
    ...
    UNIQUE(session_id, player_guid)  -- One player per session max
)

-- Weapon stats table
CREATE TABLE weapon_comprehensive_stats (
    ...
    UNIQUE(session_id, player_guid, weapon_name)  -- One weapon per player per session
)
```
...
**What happens:**
1. Manually delete all sessions/players/weapons
...
# Try insert again (same session_id, same player_guid)
cursor.execute('INSERT INTO player_comprehensive_stats (...) VALUES (...)')
