...
def get_or_create_lineup(
    ...
    if row:
        ...
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (lineup_hash, player_guids_json, player_count, session_date, session_date)
        )
...
def backfill_session(
    ...
    if len(teams) != 2:
        print(f"  ⚠️  Skipping {session_date}: expected 2 teams, got {len(teams)}")
        return False
    ...
    team_1_lineup_id = get_or_create_lineup(cursor, team_1_guids, session_date)
    team_2_lineup_id = get_or_create_lineup(cursor, team_2_guids, session_date)
    ...
    if not scoring_result:
        print(f"  ⚠️  No scoring data for {session_date}")
        return False
    ...
    cursor.execute(
        "SELECT id FROM session_results WHERE session_date = ?",
        (session_date,)
    )
    if cursor.fetchone():
        print(f"  ⏭️  Skipping {session_date}: already exists")
        return False
    ...
    cursor.execute(
        ...
        (session_date, team_1_lineup_id, team_2_lineup_id, 
    ...
    print(f"  ✅ {session_date}: {team_1_name} {team_1_score} - {team_2_score} {team_2_name} (Winner: {winner})")
    return True
...
def backfill_all_sessions():
    ...
    for session_date in session_dates:
        try:
            ...
            if backfill_session(cursor, session_date, team_data):
                success_count += 1
                conn.commit()
            else:
                skip_count += 1
    ...
    cursor.execute("""
        SELECT player_count, total_sessions, total_wins, total_losses, total_ties
        FROM team_lineups
