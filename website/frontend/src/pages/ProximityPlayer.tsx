        ...
        {Object.entries(player.breakdown).map(([catKey, metrics]) => (
          <div key={catKey}>
            ...
            {Object.entries(metrics).map(([mk, m]) => (
              <div key={mk} className={`flex items-center gap-1 text-[10px] mb-0.5${m.retired_in ? ' opacity-50' : ''}`}>
                <span className="text-slate-500 w-24 truncate" title={m.retired_in ? `not scored since ${m.retired_in}` : undefined}>
