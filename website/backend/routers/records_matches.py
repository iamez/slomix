    ...
    query = """
        SELECT * FROM (
            SELECT DISTINCT ON (player_name)
                ...
                most_useful_kills,
                bullets_fired,
                LEAST(COALESCE(time_dead_minutes, 0), time_played_seconds / 60.0) AS time_dead_minutes,
                denied_playtime,
                double_kills,
                triple_kills,
                quad_kills,
...
async def get_round_viz(
    ...
    rows = await db.fetch_all(
        ...
        SELECT player_name, player_guid, kills, deaths,
               ...
               ROUND(LEAST(COALESCE(time_dead_minutes, 0) * 60, time_played_seconds))::int AS time_dead_seconds,
