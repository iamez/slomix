...
class C0RNP0RN3StatsParser:
    ...
    def __init__(self, round_match_window_minutes: int = 45, *, allow_legacy_r1_fallback: bool = True):
        """
        ...
        Args:
            ...
            allow_legacy_r1_fallback: Search cwd/local_stats as well as the input
                                     directory. Disable for a dedicated runtime spool.
        """
        # R1-R2 matching window (configurable, was hardcoded as 30)
        self.round_match_window_minutes = round_match_window_minutes
        self.allow_legacy_r1_fallback = allow_legacy_r1_fallback
    ...
    def find_corresponding_round_1_file(self, round_2_file_path: str) -> str | None:
        ...
        if self.allow_legacy_r1_fallback and not directory.endswith("local_stats"):
        ...
        for search_dir in search_dirs:
            if os.path.exists(search_dir):
                ...
                if os.path.exists(exact_path) and (self.allow_legacy_r1_fallback or (
                    os.path.isfile(exact_path) and not os.path.islink(exact_path)
                )):
        ...
        for r1_file in potential_files:
            if not self.allow_legacy_r1_fallback and (
                os.path.islink(r1_file) or not os.path.isfile(r1_file)
            ):
                continue
