# Hybrid File Processing Helper Methods
# These methods should be added to the UltimateBot class in ultimate_bot.py
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute('''
                ...
                WHERE session_date = ? 
                  ...
                  AND round_number = ?
                LIMIT 1
            ''', (timestamp, file_info['map_name'], file_info['round_number']))
...
async def _mark_file_processed(self, filename, success=True, error_msg=None):
    """
...
async def sync_local_files_to_processed_table(self):
    """
    One-time sync: Add all existing local_stats files to processed_files table
    
    Call this during bot startup to populate the table with already-downloaded files
    """
    try:
        ...
        logger.info(f"🔄 Syncing {len(files)} local files to processed_files table...")
        ...
        if synced > 0:
            logger.info(f"✅ Synced {synced} local files to processed_files table")
