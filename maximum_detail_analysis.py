...
def get_players_for_round(conn, session_date, map_name, round_num):
...
def analyze_maximum_detail(date, db_path):
    ...
    for round_data in rounds:
        ...
        # Session date (timestamp from stat filename embedded in session_date)
        session_date = round_data['session_date']
        # session_date format: YYYY-MM-DD-HHMMSS
        # Extract original stat filename from session_date
        stat_file_pattern = f"{session_date}-{map_name}-round-{round_num}-stats.txt"
        
        print(f"║ 📄 SOURCE FILE (derived): {stat_file_pattern:<100} ║")
        print(f"║{'':140}║")
        
        # Session date (timestamp)
        print(f"║ 🕐 SESSION TIMESTAMP: {session_date:<118} ║")
    ...
    for map_id in sorted(map_groups.keys()):
        ...
        if has_r1 and has_r2:
            ...
            print(f"   R1: DB#{r1['id']:>4} @ {r1['session_date']} - Winner: Team {r1['winner_team']}")
            print(f"   R2: DB#{r2['id']:>4} @ {r2['session_date']} - Winner: Team {r2['winner_team']}")
            ...
            for r in rounds_in_map:
                print(f"   R{r['round_number']}: DB#{r['id']:>4} @ {r['session_date']} - MISSING PAIR!")
