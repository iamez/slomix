...
def get_db_session(round_date, map_name, round_num):
    ...
    c.execute("""
        SELECT 
            id, round_date, map_name, round_number,
...
def cross_reference():
    ...
    for round_date, filepath in files:
        ...
        # Get database data
        db_data = get_db_session(round_date, header['map_name'], header['round_number'])
        
        if not db_data:
            print(f"\n❌ DATABASE: Session NOT FOUND!")
            print(f"   Looking for: {round_date}, {header['map_name']}, R{header['round_number']}")
            continue
