...
for session_id, session_date, map_name, round_num in sessions:
    print(f"\n{'=' * 100}")
    print(f"Session {session_id}: {session_date} - {map_name} Round {round_num}")
    print("=" * 100)
    
    # Find the corresponding raw file
    raw_files = [f for f in os.listdir('bot/local_stats') 
                 if map_name in f and f'round-{round_num}.txt' in f and session_date[:10] in f]
    
    if not raw_files:
        print(f"  WARNING: No raw file found for {session_date} {map_name}")
        continue
