...
JOIN sessions ON player_comprehensive_stats.session_id = sessions.id
GROUP BY session_date
```
...
### 5. **!last_session Command - Team Query Errors**
...
CREATE TABLE session_teams (
    id INTEGER PRIMARY KEY,
    session_id INTEGER NOT NULL,    -- Not 'session'
...
**Files Changed:** `bot/ultimate_bot.py` (!last_session command, team subcommands)

**Verification:**
```bash
!last_session            # ✅ Shows full session with teams
!last_session team       # ✅ Shows team breakdown
!last_session team 1     # ✅ Shows specific team stats  
!last_session scoring    # ✅ Shows stopwatch team scores
```
...
!last_session team      # Test bug fix #5 ✅
```
...
SELECT session_id, team_id FROM session_teams LIMIT 1;
```
...
- ✅ `!last_session` - Complex team queries fixed
...
2. **Column Name Consistency**  
   Use consistent naming: `session_id` not `session`, `team_id` not `team`
...
- `LAST_SESSION_RESTRUCTURE.md` - !last_session command changes
...
!last_session team        # Bug #5: team queries
