...
cursor.execute("""
    SELECT s2.id, s2.session_date, s2.map_name, s2.map_id,
           s1.id as r1_id, s1.session_date as r1_date
    FROM sessions s2
    JOIN sessions s1 ON s1.map_id = s2.map_id AND s1.round_number = 1
    WHERE s2.round_number = 2
    AND SUBSTR(s2.session_date, 12, 2) = '00'  -- R2 hour is 00
    AND CAST(SUBSTR(s2.session_date, 12, 6) AS INTEGER) <= 3000  -- R2 time <= 00:30:00
    AND SUBSTR(s1.session_date, 12, 2) = '23'  -- R1 hour is 23
    AND SUBSTR(s1.session_date, 1, 10) != SUBSTR(s2.session_date, 1, 10)  -- Different dates
""")
...
for r2_id, r2_date, map_name, map_id, r1_id, r1_date in to_fix:
    ...
    # Build new R2 session_date: R1's date + R2's time
...
print("\nNow !last_session will correctly show only the Nov 2 evening session")
