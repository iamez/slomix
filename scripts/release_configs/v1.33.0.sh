# Release config for v1.33.0 — 0-0 TIE scoring fix (stale session_teams
...
RELEASE_NOTES="Fix real-match scoring read as 0-0 PERFECT TIE (stale bot session_teams roster on a reused gaming_session_id now rejected → auto-detect). Website: webp map thumbnails (18MB→316KB), defer lucide/chart.js, listener-leak guard, Record Book bot filter, midnight session range, Cloudflare Insights CSP, live current-state reducer + roster panel (A0/A1). No new migrations vs v1.32.0."
