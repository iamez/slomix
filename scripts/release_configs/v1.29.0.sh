# Release config for v1.29.0 - web triage and linkage-integrity fixes.
# shellcheck shell=bash
# shellcheck disable=SC2034
#
# Production can still upgrade from v1.25.0, and apply_migrations.py --only
# refuses when any pending migration is omitted. Keep the complete 045+
# range here; already-ledgered migrations are skipped. Before deploying,
...
RELEASE_NOTES="Web triage remediation: exact Lua round linking, guarded historical repair, and one-Lua-row-per-round enforcement."
