# Release config for v1.36.0 — identity MERGE (Phase 3): canonical_guid() folds opted-in merged alts; sick-leave attribution (073/074) included
...
RELEASE_NOTES="Phase 3 identity merge: canonical_guid() (migration 075) folds a player opted-in merged alt guids into their main identity in aggregations that GROUP BY canonical_guid(player_guid). Leaderboard retrofitted first; other surfaces follow. No behaviour change until a player runs !bolniska merge. Includes sick-leave 073/074."