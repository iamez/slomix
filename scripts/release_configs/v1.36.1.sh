# Release config for v1.36.1 — Smart Stats trust fixes, NO new migrations.
...
RELEASE_NOTES="Smart Stats trust fixes (code-only, no new migrations). Story header 'Kills' now shows the real session total (SUM of player_comprehensive_stats) gated by the same round-validity filter the scope uses, instead of the proximity-tracked KIS subset that read 61 for a 507-kill session (#709). Adds the Data Trust invariant gate — metamorphic conservation/exclusion/scope/bounds checks over the storytelling panels, CI + on-demand scripts/data_trust_check.py (#710)."