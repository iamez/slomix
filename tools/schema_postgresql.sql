...
CREATE INDEX IF NOT EXISTS idx_proximity_shot_fired_round_lookup_unlinked
    ...
    WHERE round_id IS NULL;
-- The discovery leg constrains neither map_name nor round_number, only the
-- six-hour cutoff, so it needs a recency-led index of its own (migration 069).
CREATE INDEX IF NOT EXISTS idx_proximity_shot_fired_unlinked_recent
    ON proximity_shot_fired (round_start_unix, session_date)
    WHERE round_id IS NULL;
-- The mismatch leg joins rounds on round_id, so it cannot use the partial
-- index above. Led by round_start_unix because the leg's selective predicate
-- is the six-hour recency cutoff, not the join key (migration 069).
