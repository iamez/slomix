...
def detect_teams_for_map(cursor, session_date: str, map_name: str) -> Dict:
    ...
    """, (session_date, map_name, rounds[0]))
    ...
    cursor.execute("""
        ...
        FROM player_comprehensive_stats
        WHERE session_date = ? AND map_name = ? AND round_number = ?
    """, (session_date, map_name, rounds[1]))
...
def detect_session_teams(session_date: str):
    ...
    print("="*80)
    print(f"SESSION TEAM DETECTION: {session_date}")
    print("="*80)
    ...
    map_results = []
    for map_name in maps:
        result = detect_teams_for_map(cursor, session_date, map_name)
        if result:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python correct_team_detector.py <session_date>")
        print("Example: python correct_team_detector.py 2024-10-28")
        sys.exit(1)
    
    session_date = sys.argv[1]
    result = detect_session_teams(session_date)
