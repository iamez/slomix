...
def get_r1_stats(session_date, map_name):
    """Get R1 stats for differential calculation"""
    file_path = Path('local_stats') / f"{session_date}-{map_name}-round-1.txt"
    if file_path.exists():
        return parse_raw_file(file_path)
    return {}

def analyze_session(session_id, session_date, map_name, round_num):
    """Analyze one session for missing fields"""
    
    # Get database stats
    conn = sqlite3.connect('bot/etlegacy_production.db')
    cursor = conn.cursor()
    
    # Query by session_id (which links to sessions.id)
    ...
    r1_players = {}
    if round_num == 2:
        r1_players = get_r1_stats(session_date, map_name)
    ...
    for guid in db_players.keys():
        ...
        for field in FIELD_MAPPING.values():
            ...
            if db_val == 0 and expected_val > 0:
                mismatches.append({
                    'session': f"{session_date} {map_name} R{round_num}",
                    'guid': guid,
                    'field': field,
                    'db_value': db_val,
                    'raw_value': raw_val,
...
def main():
    ...
    for session_id, session_date, map_name, round_num in sessions:
        mismatches = analyze_session(session_id, session_date, map_name, round_num)
