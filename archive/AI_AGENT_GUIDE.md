...
- **[DATABASE_REBUILD_QUICKSTART.md](../DATABASE_REBUILD_QUICKSTART.md)** - 5-step rebuild process
- **[DATABASE_REBUILD_TROUBLESHOOTING.md](../DATABASE_REBUILD_TROUBLESHOOTING.md)** - Schema issues & fixes
- **validate_schema.py** - Pre-import validation tool (ALWAYS RUN BEFORE IMPORT)
- **check_duplicates.py** - Post-import verification tool

---

## 🚨 CRITICAL INFO (Read This First!)

### Current System (October 5, 2025):
```
...
✅ Bot Commands: 14 total (includes !sessions, !list_players, !session DATE)
...
**A**: Use the standardized rebuild process:
```powershell
# Quick version (5 steps):
python validate_schema.py                                    # Step 1: Check current state
python tools/full_database_rebuild.py                        # Step 2: Clear data
python tools/create_fresh_database.py                        # Step 3: Create schema
python validate_schema.py                                    # Step 4: Validate ⚠️ CRITICAL
$env:PYTHONIOENCODING='utf-8'; python tools/simple_bulk_import.py  # Step 5: Import

# For detailed guide, see:
# - DATABASE_REBUILD_QUICKSTART.md (step-by-step)
# - DATABASE_REBUILD_TROUBLESHOOTING.md (if issues occur)
```
...
```
!last_session              # Shows most recent gaming session with full details
!session [date]            # Shows full day summary for specific date
                          # Examples: !session 2025-10-02  or  !session 2025 10 2
!sessions [month]          # Lists all gaming sessions, optionally by month
                          # Examples: !sessions october  or  !sessions 10
```

### Player Commands:
```
!stats [player/@mention]   # Show player statistics
                          # !stats            → Your own stats (if linked)
                          # !stats vid        → Search by name
                          # !stats @vid       → Search by Discord mention
!list_players [filter]     # List all players with Discord link status
...
```
!link [name/GUID/@user]    # Link Discord account to ET:Legacy GUID
                          # !link                    → Interactive linking
                          # !link carniee            → Link by player name
                          # !link D8423F90           → Link by GUID
                          # !link @user D8423F90     → Admin linking (requires permissions)
!unlink                    # Unlink your Discord account
!select <number>           # Alternative to reaction buttons in !link
```

### Leaderboard Commands:
```
!leaderboard <type> [page] # Show leaderboards
                          # Types: kills, kd, dpm, acc, hs, revives, assists,
                          #        dynamites, objectives, gibs, syringes, grenades
                          # !leaderboard kills       → Page 1
                          # !leaderboard dpm 3       → Page 3
                          # !lb kills               → Short alias
```
...
- `!leaderboard` → !lb
- `!last_session` → (no alias)

### Recent Updates (Session 7 - October 5):
- ✅ Added `!sessions` command for browsing sessions by month
- ✅ Added `!list_players` command to show Discord link status
- ✅ Fixed `!session DATE` to show full day aggregation (was showing single round)
