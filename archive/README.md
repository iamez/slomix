...
- 🤖 **14 Commands** - Session browsing, player stats, linking, leaderboards
- 👥 **Social First** - @mention friends, compare stats, see who played
- 🏆 **Competitive** - Rankings, leaderboards (12 types), MVPs, awards
...
### 3. **Session Tracking & Discovery**
```
!last_session           → Your most recent match (full details)
!session 2025-10-02     → View specific date (full day summary)
!sessions october       → Browse sessions by month
!list_players linked    → See who's connected to Discord
!leaderboard dpm        → Top players rankings (12 types)
```

**New in Session 7** (Oct 5, 2025):
- ✅ Browse sessions by month with `!sessions`
- ✅ See Discord link status with `!list_players`
- ✅ Fixed `!session` to show full day aggregation
...
## 📋 Main Commands (14 Total)

### Session & Statistics
- `!last_session` - View most recent gaming session with full details
- `!session [date]` - Show full day summary for specific date (e.g., `!session 2025-10-02`)
- `!sessions [month]` - List all gaming sessions, optionally by month (e.g., `!sessions october`)

### Player Statistics
- `!stats [player/@mention]` - View player statistics (your own, by name, or @mention)
...
- `!select <number>` - Alternative to reaction buttons

### Leaderboards
- `!leaderboard <type> [page]` - View leaderboards (kills, kd, dpm, acc, hs, revives, assists, dynamites, objectives, gibs, syringes, grenades)

### Utility
- `!ping` - Test bot responsiveness
- `!help` - Show available commands

**Aliases**: `!lb` (leaderboard), `!ls` (sessions), `!lp` (list_players)
