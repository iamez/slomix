...
cursor.execute("DELETE FROM sessions WHERE session_date LIKE 'tmp%'")
deleted = cursor.rowcount
print(f"Deleted {deleted} sessions")

# Also delete associated player stats
cursor.execute("DELETE FROM player_comprehensive_stats WHERE session_id NOT IN (SELECT id FROM sessions)")
