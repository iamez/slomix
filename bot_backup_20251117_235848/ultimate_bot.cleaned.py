...
class ETLegacyCommands(commands.Cog):
    ...
    async def last_session(self, ctx):
        ...
        try:
            async with aiosqlite.connect(self.bot.db_path) as db:
                ...
                # === EARLY ROUTING FOR !last_session SUBCOMMANDS ===
                # Short-circuit handling for compact/targeted subcommands so the
                # full report (many queries/embeds) isn't executed.
                if subcommand:
                    sc = subcommand.lower()
                    if sc in ("help", "h"):
                        help_msg = (
                            "Available !last_session subcommands: ``help``, ``top``, ``obj``, ``combat``, ``support``, ``sprees``, ``weapons``\n"
                            "Use e.g. `!last_session top` to see the compact Top/Leaderboard view."
                        )
                        await ctx.send(help_msg)
                        return
                    if sc in ("top", "t"):
                        await self._last_session_top_view(ctx, session_ids, latest_date)
                        return
                    if sc in ("obj", "objectives"):
                        await self._last_session_obj_view(ctx, session_ids, latest_date)
                        return
                    if sc == "combat":
                        await self._last_session_combat_view(ctx, session_ids, latest_date)
                        return
                    if sc == "support":
                        await self._last_session_support_view(ctx, session_ids, latest_date)
                        return
                    if sc == "sprees":
                        await self._last_session_sprees_view(ctx, session_ids, latest_date)
                        return
                    if sc == "weapons":
                        await self._last_session_weapons_view(ctx, session_ids, latest_date)
                        return
    ...
    # ---------------------- compact helpers for !last_session ----------------------
    async def _compute_session_aggregates(self, session_ids):
        """Return (player_count, total_maps, total_rounds) for given session_ids."""
        try:
            if not session_ids:
                return 0, 0, 0
            session_ids_str = ','.join('?' * len(session_ids))
            async with aiosqlite.connect(self.bot.db_path) as db:
                # player count
                q = f"SELECT COUNT(DISTINCT player_guid) FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str})"
                async with db.execute(q, session_ids) as cur:
                    player_count = (await cur.fetchone())[0] or 0

                # total maps (distinct map_name across sessions)
                q2 = f"SELECT COUNT(DISTINCT map_name) FROM sessions WHERE id IN ({session_ids_str})"
                async with db.execute(q2, session_ids) as cur:
                    total_maps = (await cur.fetchone())[0] or 0

            total_rounds = len(session_ids)
            return player_count, total_maps, total_rounds
        except Exception:
            return 0, 0, len(session_ids or [])

    async def _last_session_top_view(self, ctx, session_ids, latest_date=None):
        """Compact Top/Leaderboard view for the last session(s)."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, SUM(kills) as total_kills FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name ORDER BY total_kills DESC LIMIT 10"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            embed = discord.Embed(
                title=f"🏆 Top Players • {latest_date or 'Latest'}",
                description=f"Top leaderboard • {total_maps} maps • {total_rounds} rounds • {player_count} players",
                color=0x5865F2,
            )
            for i, r in enumerate(rows, 1):
                name, kills = r[0], r[1] or 0
                embed.add_field(name=f"{i}. {name}", value=f"`{kills}K`", inline=True)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_top_view")
            await ctx.send(f"❌ Error building top view: {e}")

    async def _last_session_obj_view(self, ctx, session_ids, latest_date=None):
        """Compact Objectives & Support view (returns a compact embed)."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, SUM(xp) as xp, SUM(times_revived) as revived FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name ORDER BY xp DESC LIMIT 8"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            embed = discord.Embed(
                title=f"🎯 Objective & Support - {latest_date or 'Latest'}",
                description=f"Top objective contributors • {player_count} players",
                color=0x00D166,
            )
            for i, r in enumerate(rows, 1):
                name, xp, revived = r[0], r[1] or 0, r[2] or 0
                txt = f"XP: `{xp}` • Revived: `{revived}`"
                embed.add_field(name=f"{i}. {name}", value=txt, inline=False)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_obj_view")
            await ctx.send(f"❌ Error building objectives view: {e}")

    async def _last_session_combat_view(self, ctx, session_ids, latest_date=None):
        """Compact combat-focused stats (kills/deaths/dpm)."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, SUM(kills) as kills, SUM(deaths) as deaths FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name ORDER BY kills DESC LIMIT 10"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            embed = discord.Embed(title=f"⚔️ Combat Leaders - {latest_date or 'Latest'}", color=0xED4245)
            for i, r in enumerate(rows, 1):
                name, kills, deaths = r[0], r[1] or 0, r[2] or 0
                kd = (kills / deaths) if deaths > 0 else kills
                embed.add_field(name=f"{i}. {name}", value=f"`{kills}K/{deaths}D ({kd:.2f})`", inline=False)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_combat_view")
            await ctx.send(f"❌ Error building combat view: {e}")

    async def _last_session_support_view(self, ctx, session_ids, latest_date=None):
        """Compact support-oriented stats (revives, assists)."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, SUM(times_revived) as revived, SUM(kill_assists) as assists FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name ORDER BY revived DESC LIMIT 8"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            embed = discord.Embed(title=f"💉 Support Leaders - {latest_date or 'Latest'}", color=0x00D166)
            for i, r in enumerate(rows, 1):
                name, revived, assists = r[0], r[1] or 0, r[2] or 0
                embed.add_field(name=f"{i}. {name}", value=f"Revived: `{revived}` • Assists: `{assists}`", inline=False)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_support_view")
            await ctx.send(f"❌ Error building support view: {e}")

    async def _last_session_sprees_view(self, ctx, session_ids, latest_date=None):
        """Compact spree/killstreak stats."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, MAX(killing_spree_best) as best_spree, MAX(death_spree_worst) as worst_spree FROM player_comprehensive_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name ORDER BY best_spree DESC LIMIT 8"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            embed = discord.Embed(title=f"🔥 Spree Highlights - {latest_date or 'Latest'}", color=0xFEE75C)
            for i, r in enumerate(rows, 1):
                name, best, worst = r[0], r[1] or 0, r[2] or 0
                embed.add_field(name=f"{i}. {name}", value=f"Best: `{best}` • Worst: `{worst}`", inline=False)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_sprees_view")
            await ctx.send(f"❌ Error building sprees view: {e}")

    async def _last_session_weapons_view(self, ctx, session_ids, latest_date=None):
        """Compact weapon mastery breakdown (top weapons per player)."""
        player_count, total_maps, total_rounds = await self._compute_session_aggregates(session_ids)
        try:
            session_ids_str = ','.join('?' * len(session_ids)) if session_ids else ''
            async with aiosqlite.connect(self.bot.db_path) as db:
                q = f"SELECT clean_name, weapon, SUM(kills) as kills FROM player_weapon_stats WHERE session_id IN ({session_ids_str}) GROUP BY clean_name, weapon ORDER BY kills DESC LIMIT 20"
                async with db.execute(q, session_ids) as cur:
                    rows = await cur.fetchall()

            # Group top weapons by player for a compact view
            by_player = {}
            for name, weapon, kills in rows:
                by_player.setdefault(name, []).append((weapon, kills))

            embed = discord.Embed(title=f"🔫 Weapon Mastery - {latest_date or 'Latest'}", color=0x5865F2)
            for i, (player, weapons) in enumerate(list(by_player.items())[:8], 1):
                txt = "\n".join([f"**{w}**: `{k}K`" for w, k in weapons[:3]])
                embed.add_field(name=f"{i}. {player}", value=txt or "No data", inline=False)
            await ctx.send(embed=embed)
        except Exception as e:
            logger.exception("Error in _last_session_weapons_view")
            await ctx.send(f"❌ Error building weapons view: {e}")
