...
def get_db_session_detailed(session_date, map_name, round_num):
    ...
    query = f"SELECT {', '.join(session_columns)} FROM sessions WHERE session_date = ? AND map_name = ? AND round_number = ?"
    c.execute(query, (session_date, map_name, round_num))
    ...
    query = f"SELECT {', '.join(player_columns)} FROM player_comprehensive_stats WHERE session_id = ? LIMIT 1"
...
def map_fields():
    ...
    session_date = '2025-10-28-212120'
    ...
    if not db_data:
        print("❌ Database session not found")
        return
