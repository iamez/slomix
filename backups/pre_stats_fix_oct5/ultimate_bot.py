    ...
    async def session(self, ctx, date_filter: str = None):
        ...
        try:
            ...
            if top_players:
                ...
                embed.add_field(name="🏆 Top 5 Players", value=top_text, inline=False)
            ...
            if len(team_stats) > 1:
                ...
                embed.add_field(name="⚔️ Team Statistics", value=team_text, inline=False)

            # Show count of other sessions
            if len(sessions) > 1:
                other_sessions = "\\n".join(
                    [f"• Session #{s[0]}: {s[2]} (Round {s[3]})" for s in sessions[1:]]
                )
                embed.add_field(
                    name=f"📋 Other Sessions on {date_filter}", value=other_sessions, inline=False
                )
    ...
    async def last_session(self, ctx):
        ...
        try:
            ...
            if axis_mvp_stats:
                ...
                embed2.add_field(name=f"🔴 {team_1_name} MVP", value=axis_mvp_text, inline=True)
            ...
            if allies_mvp_stats:
                ...
                embed2.add_field(name=f"🔵 {team_2_name} MVP", value=allies_mvp_text, inline=True)
            ...
            # Axis team with player count
            if axis_players:
                axis_text = f"**{len(axis_players)} players**\n\n"
                for i, (player, rounds, swapped) in enumerate(axis_players[:15], 1):
                    swap_indicator = " 🔄" if swapped else ""
                    axis_text += f"{i}. {player}{swap_indicator}\n"
                if len(axis_players) > 15:
                    more_count = len(axis_players) - 15
                    axis_text += f"\n*...and {more_count} more*"
                embed3.add_field(
                    name=f"🔴 {team_1_name} Roster", value=axis_text.rstrip(), inline=True
                )

            # Allies team with player count
            if allies_players:
                allies_text = f"**{len(allies_players)} players**\n\n"
                for i, (player, rounds, swapped) in enumerate(allies_players[:15], 1):
                    swap_indicator = " 🔄" if swapped else ""
                    allies_text += f"{i}. {player}{swap_indicator}\n"
                if len(allies_players) > 15:
                    more_count = len(allies_players) - 15
                    allies_text += f"\n*...and {more_count} more*"
                embed3.add_field(
                    name=f"🔵 {team_2_name} Roster", value=allies_text.rstrip(), inline=True
                )
