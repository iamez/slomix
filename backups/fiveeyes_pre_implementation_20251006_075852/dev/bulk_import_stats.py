...
class BulkStatsImporter:
    ...
    def process_single_file(self, file_path: Path) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.debug(f"✅ Processed {filename}: {len(players)} players, session {session_id}")
    ...
    def import_all_files(self, year_filter: Optional[int] = None, 
        ...
        """
        logger.info("="*70)
        logger.info("🚀 BULK STATS IMPORT - STARTING")
        logger.info("="*70)
        ...
        if year_filter:
            ...
            logger.info(f"📅 Filtered to {year_filter}: {len(all_files)} files")
    ...
    def show_summary(self):
        ...
        logger.info(f"   Skipped:   {self.stats['skipped_files']} ⏭️")
