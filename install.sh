...
setup_python_venv() {
    ...
    if [ "$MODE" = "env-only" ] || [ "$SKIP_GIT_CLONE" = true ]; then
        # Use current directory or existing DEPLOY_DIR if it exists
        if [ -d "$DEPLOY_DIR" ] && [ "$MODE" != "env-only" ]; then
            WORK_DIR="$DEPLOY_DIR"
        else
            WORK_DIR="$(pwd)"
        fi
...
configure_bot() {
    ...
    # --env-only is unambiguously dev (documented as such); --full/--vps could
    # be either a dev box or the production VPS, so this script does not
    # guess — it is left for the operator to set explicitly, and the bot
    # refuses to start without it (docs/research/ENVIRONMENT_IDENTITY_RCA_2026-08-08.md).
    if [[ "$MODE" == "env-only" ]]; then
        BOT_ENVIRONMENT_LINE="BOT_ENVIRONMENT=dev"
    else
        BOT_ENVIRONMENT_LINE="# BOT_ENVIRONMENT=dev_or_production  # <-- SET THIS before starting the bot"
        print_warning "BOT_ENVIRONMENT not set automatically for --full/--vps installs — set it to 'dev' or 'production' in .env before starting the bot"
    fi
...
# Must be "dev" or "production" — the bot will not start without it.
# See docs/research/ENVIRONMENT_IDENTITY_RCA_2026-08-08.md.
$BOT_ENVIRONMENT_LINE
...
EOF
    ...
    chown $SERVICE_USER:$SERVICE_USER "$WORK_DIR/.env" 2>/dev/null || true
...
show_completion_summary() {
    ...
    if [ "$MODE" = "env-only" ]; then
        ...
        echo ""
        echo -e "${BOLD}Virtual environment created at:${NC}"
        echo "  $WORK_DIR/$VENV_DIR"
        echo ""
        echo -e "${BOLD}To activate the virtual environment:${NC}"
        echo "  source $VENV_DIR/bin/activate"
        echo ""
        echo -e "${BOLD}To install packages:${NC}"
        echo "  $VENV_DIR/bin/pip install <package>"
        echo ""
        if [ -f "requirements.txt" ]; then
            echo -e "${BOLD}Installed from requirements.txt:${NC}"
            echo "  $(cat requirements.txt | wc -l) packages"
            echo ""
        fi
...
main() {
    ...
    if [ "$INTERACTION_MODE" = "auto" ]; then
        echo -e "${BOLD}Interaction: ${GREEN}auto${NC} (non-interactive)"
    else
        echo -e "${BOLD}Interaction: ${GREEN}interactive${NC} (with prompts)"
    fi
    echo ""
