...
class FixedTeamDetector:
    ...
    def get_deduplicated_round_data(self, session_date: str, round_number: int) -> List[Dict]:
        """
        ...
        cursor.execute("""
            WITH RankedStats AS (
                ...
                WHERE session_date = ? AND round_number = ?
            )
            SELECT 
                player_guid,
                player_name,
    ...
    def analyze_session_stopwatch(self, session_date: str) -> Dict:
        """
        ...
        print(f"{'='*80}")
        print(f"STOPWATCH SESSION ANALYSIS: {session_date}")
        print(f"{'='*80}\n")
        ...
        for round_num in rounds:
            data = self.get_deduplicated_round_data(session_date, round_num)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python fixed_team_detector.py <session_date>")
    ...
    result = detector.analyze_session_stopwatch(session_date)
