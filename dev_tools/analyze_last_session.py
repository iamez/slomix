...
# Get the session
cursor.execute("""
    SELECT DISTINCT round_date, map_name, round_number
    FROM player_comprehensive_stats
    WHERE round_date IN ('2025-11-01', '2025-11-02')
    ORDER BY round_date, map_name, round_number
""")

session_data = cursor.fetchall()

# Group by round_date and map
...
for (round_date, map_name), rounds in sorted(by_session_map.items()):
    map_count += 1
    
    print(f"\n{'#'*80}")
    print(f"MAP {map_count}: {map_name} ({round_date})")
    print(f"{'#'*80}")
            ...
            if left_players:
                ...
                for guid in left_players:
                    ...
                    cursor.execute("""
                        SELECT player_name
                        FROM player_comprehensive_stats
                        WHERE round_date = ? AND map_name = ? AND player_guid = ?
                        LIMIT 1
                    """, (round_date, map_name, guid))
...
for guid, name in all_players:
    ...
    cursor.execute("""
        ...
        FROM player_comprehensive_stats
        WHERE round_date IN ('2025-11-01', '2025-11-02') AND player_guid = ?
    """, (guid,))
