"""
Comprehensive validation of Nov 2 session data
Compares ALL fields from raw stats files against database
"""
...
def parse_raw_file(filepath):
    ...
    for player in result.get('players', []):
        guid = player.get('guid', '')[:8]  # Ensure 8 chars
        if guid:
...
def compare_round(filepath, round_id, verbose=False):
    ...
    if verbose:
        print(f"  Raw GUIDs: {sorted(raw_players.keys())}")
        print(f"  DB GUIDs:  {sorted(db_players.keys())}")
    ...
    for guid in sorted(all_guids):
        ...
        if verbose and all_weapons:
            print(f"\n  DEBUG {raw.get('name')} ({guid}):")
            print(f"    Raw weapons: {sorted(raw.get('weapon_stats', {}).keys())}")
            print(f"    DB weapons:  {sorted(db.get('weapons', {}).keys())}")
...
def main():
    ...
    if len(files) != len(SESSION_IDS):
        print(f"\n[WARNING] File count ({len(files)}) != Session count ({len(SESSION_IDS)})")
    ...
    for i, filepath in enumerate(files):
        if i >= len(SESSION_IDS):
            print(f"\n[ERROR] No round ID for file {filepath.name}")
            break
        
        round_id = SESSION_IDS[i]
        passed, failed = compare_round(filepath, round_id, verbose=(i==0))  # Verbose only for first round
        total_passed += passed
        total_failed += failed
