...
# Check round_teams for Nov 1
cursor.execute("SELECT COUNT(*) FROM session_teams WHERE round_date LIKE '2025-11-01%'")
count = cursor.fetchone()[0]
print(f"session_teams records for Nov 1: {count}")

if count > 0:
    cursor.execute("""
        SELECT round_date, team_name, player_names 
        FROM session_teams 
