...
def test_detection(round_date: str, db_path: str = "bot/etlegacy_production.db"):
    """Test team detection for a session"""
    
    print("=" * 80)
    print("🎯 ADVANCED TEAM DETECTION TEST")
    print("=" * 80)
    print(f"Session Date: {round_date}")
    ...
    is_valid, reason = detector.validate_stored_teams(conn, round_date)
    ...
    if existing_teams:
        ...
        if user_input.lower() == 'y':
            success = detector.store_detected_teams(conn, round_date, result)
            if success:
...
def compare_with_old(round_date: str, db_path: str = "bot/etlegacy_production.db"):
    ...
    # Get new teams
    detector = TeamDetectorIntegration(db_path)
    result, _ = detector.detect_and_validate(conn, round_date)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_advanced_team_detection.py <round_date> [--compare]")
    ...
    success = test_detection(round_date)
    
    if success and compare:
        print()
        compare_with_old(round_date)
