...
def backfill_time_values(stats_dir='local_stats', db_path='bot/etlegacy_production.db'):
    ...
    # Group files by session_date and map_name to find R1+R2 pairs
    updates = []
    map_pairs = {}  # Key: (session_date, map_name), Value: {R1: data, R2: data}
    
    for stat_file in sorted(stat_files):
        filename = os.path.basename(stat_file)
        
        # Extract session_date from filename
        ...
        session_date = f"{parts[0]}-{parts[1]}-{parts[2]}-{parts[3]}"
    ...
    for key, rounds in map_pairs.items():
        ...
        if 1 in rounds:
            ...
            c.execute("""
                ...
                AND map_name = ?
                AND round_number = 1
            """, (original_limit, completion, session_date, map_name))
            
            if c.rowcount > 0:
                updates_count += 1
                print(f"✅ R1: {session_date} {map_name:<20} "
        ...
        if 2 in rounds:
            ...
            c.execute("""
                ...
                AND map_name = ?
                AND round_number = 2
            """, (original_limit, time_to_beat, completion, session_date, map_name))
            
            if c.rowcount > 0:
                updates_count += 1
                print(f"✅ R2: {session_date} {map_name:<20} "
