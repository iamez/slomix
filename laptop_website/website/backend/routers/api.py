...
async def get_live_session(db: DatabaseAdapter = Depends(get_db)):
    ...
    try:
        ...
        latest_query = "SELECT map_name, round_date, stopwatch_time FROM player_comprehensive_stats WHERE date(round_date) = date('now') ORDER BY round_date DESC LIMIT 1"
        latest = await db.fetch_one(latest_query)
...
async def get_leaderboard(
    ...
    # Base query parts
    where_clause = "WHERE stopwatch_time IS NOT NULL AND DATE(round_date) >= $1"
    group_by = "GROUP BY player_name"
    having = "HAVING COUNT(DISTINCT gaming_session_id) >= $2"
        ...
        query = f"""
            WITH player_stats AS (
                SELECT
                    ...
                    ROUND((SUM(damage_given)::numeric / NULLIF(SUM(stopwatch_time), 0) * 60), 2) as value,
