...
class StatsImageGenerator:
    ...
    def create_session_overview(self, session_data, top_players, team_data,
                                team_names):
        """
        ...
        draw.text((50, y_pos), title, fill=self.COLORS['text_white'],
                 font=title_font)
        y_pos += 70
        ...
        draw.text((50, y_pos), info_text, fill=self.COLORS['text_gray'],
                 font=stat_font)
        y_pos += 50
        
        # Divider line
        draw.rectangle([50, y_pos, self.width - 50, y_pos + 2],
                      fill=self.COLORS['bg_light'])
        y_pos += 30
        
        # Top 5 Players Section
        draw.text((50, y_pos), "🏆 Top 5 Players",
                 fill=self.COLORS['accent_yellow'], font=header_font)
        y_pos += 50
        ...
        for i, player in enumerate(top_players[:5]):
            ...
            draw.text((70, y_pos), name_text, fill=self.COLORS['text_white'],
                     font=stat_font)
            y_pos += 30
            ...
            draw.text((100, y_pos), stats1, fill=self.COLORS['text_gray'],
                     font=small_font)
            y_pos += 25
            ...
            draw.text((100, y_pos), stats2, fill=self.COLORS['text_dim'],
                     font=small_font)
            y_pos += 40
        ...
        draw.text((50, y_pos), title_text, fill=self.COLORS['accent_red'],
                 font=header_font)
        y_pos += 50
        ...
        draw.text((col1_x, y_pos), f"🔴 {team1_name}",
                 fill=self.COLORS['accent_red'], font=stat_font)
        y_temp = y_pos + 35
        team1_stats = (
        ...
        for line in team1_stats.split('\n'):
            draw.text((col1_x, y_temp), line, fill=self.COLORS['text_gray'],
                     font=small_font)
            y_temp += 25
        ...
        if 'mvp' in team1:
            ...
            for line in mvp_text.split('\n'):
                draw.text((col1_x, y_temp), line,
                         fill=self.COLORS['accent_green'], font=small_font)
                y_temp += 23
        
        # Team 2
        team2 = team_data['team2']
        draw.text((col2_x, y_pos), f"🔵 {team2_name}",
                 fill=self.COLORS['accent_blue'], font=stat_font)
        y_temp = y_pos + 35
        team2_stats = (
        ...
        for line in team2_stats.split('\n'):
            draw.text((col2_x, y_temp), line, fill=self.COLORS['text_gray'],
                     font=small_font)
            y_temp += 25
    ...
    def create_performance_graphs(self, player_data):
        ...
        bars1 = ax1.bar([i - width for i in x], kills_data, width,
                       label='Kills', color=self.COLORS['accent_blue'],
                       alpha=0.9)
        bars2 = ax1.bar(x, deaths_data, width,
                       label='Deaths', color=self.COLORS['accent_red'],
                       alpha=0.9)
        bars3 = ax1.bar([i + width for i in x], dpm_data, width,
                       label='DPM', color=self.COLORS['accent_yellow'],
                       alpha=0.9)
        ...
        ax1.set_xticks(x)
        ax1.set_xticklabels(names, rotation=20, ha='right', color='white',
                           fontsize=12)
        ax1.legend(facecolor=self.COLORS['bg_medium'], edgecolor='white',
        ...
        bars4 = ax2.bar([i - width/2 for i in x], kd_ratios, width,
                       label='K/D Ratio', color=self.COLORS['accent_green'],
                       alpha=0.9)
        bars5 = ax2_twin.bar([i + width/2 for i in x], accuracy_data, width,
                            label='Accuracy %',
                            color=self.COLORS['accent_pink'], alpha=0.9)
        
        ax2.set_ylabel('K/D Ratio', color='white', fontsize=14,
                      fontweight='bold')
        ax2_twin.set_ylabel('Accuracy %', color='white', fontsize=14,
                           fontweight='bold')
        ax2.set_title('Player Efficiency - K/D Ratio and Accuracy',
                     color='white', fontsize=16, fontweight='bold', pad=20)
        ax2.set_xticks(x)
        ax2.set_xticklabels(names, rotation=20, ha='right', color='white',
                           fontsize=12)
        ...
        ax2.legend(lines1 + lines2, labels1 + labels2,
                  facecolor=self.COLORS['bg_medium'], edgecolor='white',
                  labelcolor='white', fontsize=12)
    ...
    def create_weapon_mastery_image(self, player_weapons_data):
        ...
        # Title
        draw.text((50, y_pos), "🔫 Weapon Mastery Breakdown",
                 fill=self.COLORS['accent_pink'], font=title_font)
        y_pos += 60
        
        # Subtitle
        draw.text((50, y_pos), "Top weapons for each player with detailed statistics",
                 fill=self.COLORS['text_gray'], font=weapon_font)
        y_pos += 50
        
        # Divider
        draw.rectangle([50, y_pos, self.width + 150, y_pos + 2],
                      fill=self.COLORS['bg_light'])
        y_pos += 30
        ...
        for player_name, weapons in list(player_weapons_data.items())[:6]:
            ...
            draw.text((x_pos, y_pos), player_header,
                     fill=self.COLORS['text_white'], font=player_font)
            y_temp = y_pos + 35
            ...
            for weapon_idx, weapon_data in enumerate(weapons[:3]):
                ...
                color = weapon_colors[weapon_idx] if weapon_idx < len(weapon_colors) else self.COLORS['text_gray']
                weapon_text = f"  {weapon_idx + 1}. {weapon}"
                draw.text((x_pos + 10, y_temp), weapon_text,
                         fill=color, font=weapon_font)
                y_temp += 28
                
                # Stats line
                stats_text = f"     {kills} Kills  •  {acc:.1f}% ACC  •  {hs_pct:.1f}% HS ({hs}/{hits})"
                draw.text((x_pos + 10, y_temp), stats_text,
                         fill=self.COLORS['text_dim'], font=stat_font)
                y_temp += 26
        ...
        draw.text((50, y_pos), "ACC = Accuracy (Hits/Shots)  •  HS = Headshot Rate (Headshots/Hits)",
                 fill=self.COLORS['text_dim'], font=stat_font)
