...
class ETLegacyCommands(commands.Cog):
    ...
    async def last_session(self, ctx):
        ...
        try:
            async with aiosqlite.connect(self.bot.db_path) as db:
                ...
                if 1 in team_mvps:
                    ...
                    async with db.execute(query, session_ids + [player]) as cursor:
                        result = await cursor.fetchone()
                        if result:
                            axis_mvp_stats = (player, kills, result[0], result[1])
                ...
                if 2 in team_mvps:
                    ...
                    async with db.execute(query, session_ids + [player]) as cursor:
                        result = await cursor.fetchone()
                        if result:
                            allies_mvp_stats = (player, kills, result[0], result[1])
            ...
            if axis_mvp_stats:
                player, kills, dpm, deaths = axis_mvp_stats
            ...
            if allies_mvp_stats:
                player, kills, dpm, deaths = allies_mvp_stats
                kd = kills / deaths if deaths else kills
                allies_mvp_text = (
                    f"**{player}**\n"
                    f"💀 `{kd:.1f} K/D` ({kills}K/{deaths}D)\n"
            ...
            # ═══════════════════════════════════════════════════════
            # MESSAGE 5: Weapon Mastery Image
            # ═══════════════════════════════════════════════════════
            try:
                from image_generator import StatsImageGenerator
                ...
                for player, weapon, kills, hits, shots, hs in player_weapons:
                    ...
                    acc = (hits / shots * 100) if shots > 0 else 0
                    # HS% = headshots / hits (not shots!)
                    hs_pct = (hs / hits * 100) if hits > 0 else 0
                    weapon_clean = weapon.replace('WS_', '').replace('_', ' ')
                    weapon_clean = weapon_clean.title()
                    player_weapon_map[player].append(
                        (weapon_clean, kills, acc, hs_pct, hs, hits, shots)
                    )
                
                # Sort players by total kills and take top 6
                player_totals = []
                ...
                # Prepare data for top 6 players with their top 3 weapons
                weapon_mastery_data = {}
                for player, total_kills in player_totals[:6]:
                    weapon_mastery_data[player] = player_weapon_map[player][:3]
                
                # Generate weapon mastery image
                generator = StatsImageGenerator()
                weapon_img_buf = generator.create_weapon_mastery_image(weapon_mastery_data)
                
                weapon_file = discord.File(weapon_img_buf, filename='weapon_mastery.png')
                await ctx.send(
                ...
                logger.error(f"Error generating weapon mastery image: {e}", exc_info=True)
                # Fallback to simple text message
                await ctx.send("❌ Error generating weapon mastery visualization")
            
            # ═══════════════════════════════════════════════════════
            # MESSAGE 6: Objective & Support Stats (NEW!)
            # ═══════════════════════════════════════════════════════
            ...
            if awards_data:
                ...
                player_objectives = {}
                for player_name, awards_json in awards_data:
                    ...
                    player_objectives[player_name]['xp'] += awards.get('xp', 0)
                    player_objectives[player_name]['assists'] += awards.get('kill_assists', 0)
                    player_objectives[player_name]['obj_stolen'] += awards.get('objectives_stolen', 0)
                    player_objectives[player_name]['obj_returned'] += awards.get('objectives_returned', 0)
                    player_objectives[player_name]['dyn_planted'] += awards.get('dynamites_planted', 0)
                    player_objectives[player_name]['dyn_defused'] += awards.get('dynamites_defused', 0)
                    player_objectives[player_name]['revived'] += awards.get('times_revived', 0)
                    player_objectives[player_name]['multi_2x'] += awards.get('multikill_2x', 0)
                    player_objectives[player_name]['multi_3x'] += awards.get('multikill_3x', 0)
                    player_objectives[player_name]['multi_4x'] += awards.get('multikill_4x', 0)
