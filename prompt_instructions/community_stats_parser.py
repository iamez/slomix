...
class C0RNP0RN3StatsParser:
    ...
    def create_stylish_round_embed(self, stats_data: Dict[str, Any]):
        ...
        embed.set_footer(
            text="ET:Legacy Community Stats • c0rnp0rn3.lua"
        )
    ...
    def parse_round_2_with_differential(self, round_2_file_path: str) -> Dict[str, Any]:
        ...
        print(f"✅ Successfully calculated Round 2-only stats for {len(round_2_only_result['players'])} players")
        return round_2_only_result

    def calculate_round_2_differential(self, round_1_data: Dict[str, Any], 
                                     round_2_cumulative_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for r2_player in round_2_cumulative_data['players']:
            ...
            differential_player = {
                'guid': r2_player.get('guid', 'UNKNOWN'),  # FIX: Include GUID for database operations
                'name': player_name,
                'team': r2_player['team'],
                ...
                'damage_given': max(0, r2_player.get('damage_given', 0) - r1_player.get('damage_given', 0)),
                'damage_received': max(0, r2_player.get('damage_received', 0) - r1_player.get('damage_received', 0)),
                'weapon_stats': {},
            ...
            for weapon_name, r2_weapon in r2_player.get('weapon_stats', {}).items():
                ...
                if differential_weapon['shots'] > 0:
                    differential_weapon['accuracy'] = (differential_weapon['hits'] / differential_weapon['shots']) * 100
                else:
            ...
            if differential_player['deaths'] > 0:
                differential_player['kd_ratio'] = differential_player['kills'] / differential_player['deaths']
            else:
            ...
            differential_player['efficiency'] = total_kills / (total_kills + total_deaths) * 100 if (total_kills + total_deaths) > 0 else 0
    ...
    def parse_player_line(self, line: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # After weapon stats come TAB-separated fields (0-36)
            additional_stats = {}
            ...
            if extended_section:
                ...
                # Actual format has 33 TAB-separated fields
                if len(tab_fields) >= 33:  # Full extended format
                    try:
                        # Basic stats (indices 0-8)
                        additional_stats = {
                            'damage_given': int(tab_fields[0]),
                            'damage_received': int(tab_fields[1]),
                            'gibs': int(tab_fields[2]),
                            'self_kills': int(tab_fields[3]),
                            'team_gibs': int(tab_fields[4]),
                            'team_kills': int(tab_fields[5]),
                            'team_damage_given': int(tab_fields[6]),
                            'team_damage_received': int(tab_fields[7]),
                            'time_played_percent': float(tab_fields[8]),
                        }
                        
                        # Objective & Support stats (for player_stats.awards JSON)
                        # Actual format: 33 TAB-separated fields (indices 0-32)
                        objective_stats = {
                            # Fields 0-8: damage/gibs/team stats
                            'damage_given': int(tab_fields[0]),
                            'damage_received': int(tab_fields[1]),
                            'gibs': int(tab_fields[2]),
                            'team_kills': int(tab_fields[3]),
                            'time_axis_percent': float(tab_fields[4]),
                            'time_allies_percent': float(tab_fields[5]),
                            'time_spec_percent': float(tab_fields[6]),
                            'unused_7': int(tab_fields[7]),
                            'time_played_percent': float(tab_fields[8]),
                            ...
                            # Fields 20-28: bullets, DPM, time, K/D stats
                            'bullets_fired': int(tab_fields[20]),
                            'dpm': float(tab_fields[21]),
                            # Tab[22] has actual lua-rounded time in minutes!
                            # Tab[23] is always 0.0 (unused by lua)
                            'time_played_minutes': float(tab_fields[22]),
                            'tank_meatshield': float(tab_fields[24]),
                        ...
                        print(f"Warning: Could not parse all 37 fields: {e}")
