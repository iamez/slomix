...
> Update (February 18, 2026): This document is historical. Availability and notification architecture has moved to:
...
â”‚   â”‚
â”‚   â””â”€â”€ services/
â”‚       â””â”€â”€ automation/            # Automation services (integrated)
...
- âœ… `ssh_monitor.py` - Loaded and available for manual control
- âœ… `metrics_logger.py` - Loaded and used by automation reporting commands
- âœ… `health_monitor.py` - Loaded and used by health command flow
- âœ… `database_maintenance.py` - Loaded and used by maintenance commands
- âœ… `automation_commands.py` - Cog loaded at startup
...
- âœ… Endstats loop remains the primary auto SSH polling path
- âœ… Separate automation services are integrated and available via commands
- âœ… SSH monitor auto-start is intentionally disabled to avoid duplicate processing
