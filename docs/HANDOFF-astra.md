...
- Ultra pregled: `/code-review ultra <PR#>` sproži OWNER; meja **8 000 vrstic / 500 datotek na tek** → 20 draft PR-jev **#924–#943** (`review-base/NN` → `review/NN`, glava = mainovo drevo), sekani na `71de6b65`; vrstni red #924 (proximity+spiderweb+Lua) → #925 (backend routerji) → #926 (SPA lib). Ponovni rez po vsakem premiku maina je bil `scripts/review_slices.sh cut --push` — ⚠️ skripta potiska s `--force --no-verify` (§I), kar je proti AGENTS §1; **Astra ga ne poganja**, dokler owner ne izbere med obstoječo skripto (izjema zapisana v AGENTS) in nespremenljivimi posnetki ≤ 25 datotek. Nikoli mergati teh PR-jev.
...
- Skriptni reševalec git konfliktov: obdelaj VSE hunke, JSON ni proza (regeneriraj `docs/api/openapi.json` s `scripts/dump_openapi.py`), po reševanju `grep -c '^<<<<<<<'` = 0; `git add <vsaka razrešena datoteka po imenu>` (nikoli `-u`/`-A`) + commit; `git add -u` je 6. 9. sprejel markerje in tujo spremembo.
...
- `mergeStateStatus` zaostaja za resnico (BLOCKED ob zelenih checkih); merge gre IZKLJUČNO prek `~/slomix-ops/cycle.sh` (7-min premor, preverba zaostanka, niti = 0) — ne `gh pr merge` in ne `--auto`.
...
6. **`response_model` gap 217 → 0** po routerjih, z metodo, ki jo pribijajo `tests/unit/test_response_model_gap.py` in `tests/unit/test_manual_types_do_not_drift_further.py` (PR #820/#830 v git zgodovini): (1) posnemi korpus odgovorov endpointa (`scripts/record_api_corpus.py --sentinel`), (2) model = unija posnetih oblik, ničesar ne izpusti, (3) AST preveri, da handler vrača le polja modela, (4) živa meritev pred/po (isti `sha256` telesa), (5) prazen posneti seznam = NE tipiziraj (ni dokaza oblike); `tests/data/manual_type_drift.txt` na 0; vsak model = unija posnetih oblik, `exclude_unset` le kjer je odsotnost pomen.
