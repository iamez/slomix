--[[
    ...
    3. Configure webhook via cvar/env (or set placeholder below)
...
local configuration = {
    ...
    -- Resolution order: cvar -> env -> this value.
    discord_webhook_url = "REPLACE_WITH_YOUR_WEBHOOK_URL",
    discord_webhook_cvar = "slomix_discord_webhook_url",
    discord_webhook_env = "SLOMIX_DISCORD_WEBHOOK_URL",
...
local function is_valid_webhook_url(url)
    if not url or url == "" then
        return false
    end
    if url == "REPLACE_WITH_YOUR_WEBHOOK_URL" then
        return false
    end
    if string.find(url, "WEBHOOK_ID", 1, true) or string.find(url, "WEBHOOK_TOKEN", 1, true) then
        return false
    end
    if string.match(url, "^https://discord%.com/api/webhooks/") then
        return true
    end
    if string.match(url, "^https://discordapp%.com/api/webhooks/") then
        return true
    end
    return false
end

local function resolve_webhook_url()
    local cvar_name = configuration.discord_webhook_cvar or ""
    if cvar_name ~= "" then
        local cvar_value = et.trap_Cvar_Get(cvar_name)
        if is_valid_webhook_url(cvar_value) then
            return cvar_value, "cvar:" .. cvar_name
        end
    end

    local env_name = configuration.discord_webhook_env or ""
    if env_name ~= "" and os.getenv then
        local env_value = os.getenv(env_name)
        if is_valid_webhook_url(env_value) then
            return env_value, "env:" .. env_name
        end
    end

    if is_valid_webhook_url(configuration.discord_webhook_url) then
        return configuration.discord_webhook_url, "config"
    end
...
local function execute_curl_async(payload_json, webhook_url)
...
local function send_webhook()
    ...
    webhook_url = select(1, resolve_webhook_url())
    if not webhook_url then
        et.G_Print(string.format(
            "[%s] ERROR: Discord webhook URL not configured (set cvar '%s', env '%s', or configuration.discord_webhook_url)\n",
            modname,
            tostring(configuration.discord_webhook_cvar),
            tostring(configuration.discord_webhook_env)
        ))
        return
    end
    ...
    local ok_send, err = pcall(function()
        ...
        local ok, msg = execute_curl_async(payload, webhook_url)
        if not ok then
...
function et_InitGame(levelTime, randomSeed, restart)
    ...
    local webhook_url, webhook_source = resolve_webhook_url()
    if not webhook_url then
        et.G_Print(string.format(
            "[%s] WARNING: Webhook URL not configured (cvar '%s', env '%s', or configuration.discord_webhook_url)\n",
            modname,
            tostring(configuration.discord_webhook_cvar),
            tostring(configuration.discord_webhook_env)
        ))
    else
        log(string.format("Webhook URL configured via %s", webhook_source))
    end
end
