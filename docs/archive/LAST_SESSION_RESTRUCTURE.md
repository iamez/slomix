# !last_round Command Restructure
...
```
!last_round          → Clean view with 2 essential graphs only
!last_round graphs   → Detailed round/map breakdown (Graph 3)
!last_round full     → Everything including advanced stats (Graph 2 + Graph 3)
```
...
- `bot/ultimate_bot.py` - Lines 1276-3290
  - Added `subcommand` parameter to `last_round()`
...
- [x] Test `!last_round` - shows only Graph 1 and Graph 4
- [x] Test `!last_round graphs` - shows Graph 1, 3, and 4
- [x] Test `!last_round full` - shows all 4 graphs
...
October 10, 2025 - Round with user feedback incorporation
