...
class TeamManager:
    def detect_session_teams(self, db, round_date):
        ...
        return detector.get_or_detect_teams(db, round_date)
```
...
- **Scalability**: Handles 100+ players per round
...
2. **Update Last Round** - Use new detector in `bot/cogs/last_session_cog.py`
