#!/usr/bin/env python3
"""
Fixed version - Uses R1 filename for match_id extraction.
"""
...
def create_backup():
    ...
    backup_file = f"{BACKUP_DIR}/etlegacy_before_r2_fix_v2_{timestamp}.sql"
...
def get_round2_files():
    """Get all Round 2 files (including endstats)"""
    r2_files = []
    for filename in os.listdir(LOCAL_STATS):
        # Process ONLY endstats Round 2 files (already processed regular files)
        if '-round-2-endstats.txt' in filename:
...
def main():
    print("=" * 100)
    print("CORRECTED R2 FIX - Uses R1 match_id from parser")
    print("=" * 100)
    ...
    for i, (filename, filepath) in enumerate(r2_files, 1):
        ...
        try:
            ...
            # ✅ FIX: Extract match_id from R1 filename (not R2!)
            ...
            if player_updates > 0:
                print(f"  ✅ Updated {player_updates} players (match_id: {match_id})")
                fixed_count += 1
            else:
