...
PGPASSWORD="REDACTED_DB_PASSWORD" psql -h 127.0.0.1 -U etlegacy_user -d etlegacy <<'SQL'
SELECT
    'snapshot' AS marker,
    NOW() AS taken_at,
...
```bash
PGPASSWORD="REDACTED_DB_PASSWORD" psql -h 127.0.0.1 -U etlegacy_user -d etlegacy -tA -c "
SELECT COUNT(*) FROM round_correlations
...
- [ ] **Sandbox kopija** za test:
  ```bash
  PGPASSWORD="REDACTED_DB_PASSWORD" createdb -h 127.0.0.1 -U etlegacy_user etlegacy_sandbox
...
PGPASSWORD="REDACTED_DB_PASSWORD" psql -h 127.0.0.1 -U etlegacy_user -d etlegacy < backup_YYYYMMDD.sql
...
- DB: `PGPASSWORD="REDACTED_DB_PASSWORD" psql -h 127.0.0.1 -U etlegacy_user -d etlegacy`
