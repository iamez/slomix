...
## A2 — Deploy
```
./scripts/deploy_release.sh <TAG>
BASE_URL=https://www.slomix.fyi ./scripts/verify_post_deploy.sh
```
Opombe:
- Root `migrations/` NIMA novih datotek od zadnjega deploya — migracijski
  korak bo no-op. `apply_migrations.py` ima NOVO semantiko (#457): failed
  vrstice se retryjajo in status jih šteje ločeno; populated-DB-brez-trackinga
  zdaj ODKLONI apply (na prod tracking obstaja, torej brez spremembe).
- **ET Rating self-heal (C7, FYI):** prvi auto-refresh po deployu prečisti
  OMNIBOT vnose iz `player_skill_ratings` in preračuna percentile brez botov
  → ratingi se premaknejo do ±0.13, populacija ~40→~26 ljudi. Pričakovano,
  ne bug.

## A3 — Prod backfilli (owner-gated, po A1 backupu)
```
...
- s.effort vrstice same: `DELETE FROM player_skill_history WHERE scope='session'`
