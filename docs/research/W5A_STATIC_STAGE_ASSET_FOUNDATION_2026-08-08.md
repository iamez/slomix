...
- single-quoted PC `TT_LITERAL` tokens use the same escape/adjacency rules while retaining their delimiters, matching
  `PC_ReadTokenHandle`, which strips delimiters only from double-quoted `TT_STRING` tokens;
