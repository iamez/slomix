...
2. **Osi se čisto razdelijo na lastnosti igralca in lastnosti runde.**
   `stood` 0,83 · `wave` 0,81 · `clean_pick` 0,71 so stabilne pri igralcu;
   `escort` **0,00** · `crossfire` 0,00 · `objective` 0,08 · `isolation` 0,28 niso.
...
delež te med-igralske v opaženi. Ostanek je odštet po celici (mapa, stran) — ta celica **je** vloga
(Axis brani na vseh mapah razen `etl_ice`), zato dodatno odštevanje vloge samo
doda konstanto; prva izvedba je to počela in dala do 0,05 razlike (`isolation`
0,334 namesto 0,281), zato je odstranjeno. Interval iz bootstrapa po **rundah** (odvisnost, ki jo je prej
požrl split-half).

| os | zanesljivost | 95 % CI | med-igralski sd | šumni sd | najmanjša zaznavna razlika |
|---|---:|---|---:|---:|---:|
| `stood` | **0,833** | [0,74; 0,91] | 0,035 | 0,016 | 0,044 |
| `wave_z` | **0,810** | [0,70; 0,90] | 0,066 | 0,032 | 0,089 |
| `clean_pick` | **0,709** | [0,63; 0,86] | 0,024 | 0,015 | 0,043 |
| `revenge` | 0,503 | [0,27; 0,82] | 0,011 | 0,011 | 0,032 |
| `gibbed` | 0,385 | [0,09; 0,83] | 0,003 | 0,004 | 0,012 |
| `isolation` | 0,281 | [0,00; 0,74] | 0,010 | 0,015 | 0,043 |
| `objective` | 0,084 | [0,00; 0,81] | 0,003 | 0,011 | 0,030 |
| `escort` / `crossfire` / `revived` | **0,000** | [0,00; 0,79] | 0,000 | 0,007–0,013 | 0,018–0,037 |
...
⚠️ **To merilo ni isto kot v KIS auditu.** Tu je klasifikacijska točnost s CV in
lutkami za mapo, zato je vanjo vštet **večinski razred (64,11 %)** — napadalec ne
zmaga polovice rund. V `KIS_VALIDITY_AUDIT_2026-08-19.md` je merilo parni test po
rundah (izhodišče 66,93 % za golo štetje). **Absolutnih vrednosti med dokumentoma
ne primerjaj**; primerljive so samo razlike znotraj enega merila — in razlika tu
je +2,51 o. t.
...
|---|---:|---:|---|
| `stood` | 0,833 | 0,274 | **ocena igralca (KIS)** |
| `wave_z` | 0,810 | 0,156 | **ocena igralca (KIS)** |
| `clean_pick` | 0,709 | 0,074 | ocena igralca (šibek učinek) |
| `isolation` | 0,281 | 0,180 | ocena runde (PWC) |
| `gibbed` | 0,385 | 0,103 | ocena runde |
| `objective` | 0,084 | 0,314 | **ocena runde (PWC)** |
...
| `revenge` | 0,503 | 0,014 | ⛔ ven (ne napove ničesar) |
...
Ridge APM (λ = 25) na 1.938 rundah in 36 igralcih. ⚠️ Teh 1.938 je od 2.935
veljavnih rund — razlika **ni** posledica varovala »natanko dve ekipi« (to izloči
le 4 runde: 2 z eno ekipo, 2 s tremi), ampak tega, da preostale runde nimajo
`player_comprehensive_stats` vrstic v obsegu (`round_number IN (1,2)`,
`time_played_seconds > 0`). Gre za pokritost, ne za napako v združevanju: ena vrstica na rundo, +1 za
