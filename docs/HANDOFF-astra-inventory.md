...
- ~~`[S, owner]` Install the timer~~ **DONE 2026-09-07 02:41** (timer active, runs every 5 min from the run dir; state in `slomix-dev-run/logs/watchdog_state.json`). Original text: `WATCHDOG_WEBHOOK_URL` into the run-dir `.env`, then `sudo cp deploy/systemd/etlegacy-watchdog.* /etc/systemd/system/ && daemon-reload && enable --now` — `PLAN.md:113-115` — depends on: **owner** — proof: the r. 1 dry-run already showed 9/9 `ok` (disk 84.7 %, just under the 85 threshold) and `bot_streaks` `unknown`.
