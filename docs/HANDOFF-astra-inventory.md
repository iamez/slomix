...
- `[S]` `/api/rounds/{}/awards` — **in flight as PR #955** — `endpoint_gap.txt:100`, `HANDOFF-next.md:104-105` — depends on: owner decision on *where* (there is no per-round surface yet) — proof: raw `value` vs `session_awards_service`'s nicer form.
...
- `[M]` Final parity sweep re-run (32 routes × 4 viewports × anon/owner = 256 checks) — `PLAN.md:140-143`, `BACKLOG.md:43-48`, `HANDOFF-next.md:43` — depends on **RAM** (run 3 was aborted at 1.8 GB used / 196 MB free; chromium leaves ~240 MB) — proof: `AUDIT_BASE_URL=…:8056 node scripts/audit_website_browser.mjs --app --manifest`; ⛔ afterwards check `ps` and kill by PID, never `pkill -f`.
...
- `[S]` Switchover blockers, all six must hold at once — `docs/design/09 §"Merilo za dan preklopa"` — depends on **prod freeze lift (owner)**: (1) `endpoint_gap.txt` empty; (2) `parity_diff.mjs` clean on 33 routes × 4 viewports × anon/owner; (3) H3 green on all routes both projects; (4) canvas pixel-diff green on 3 rounds × 3 maps; (5) openapi snapshot matches the live app; (6) **tested on dev against a real evening of play**, not history. Cutover itself = `build:app` in `scripts/deploy_release.sh` + SPA fallback in `main.py`; rollback = revert that commit + restart `slomix-web`.
...
- `[S]` Local trap: generated `src/api/generated/openapi.d.ts` was stale twice at typecheck → `rm` before measuring, or add a `pretypecheck` — `BACKLOG.md:286-287`.
...
- `[M]` **Watchdog r. 2 — SSH probes on puran** (tailer `pgrep`, `ls -t stats/`) — `PLAN.md:116-118`, `docs/design/24_WATCHDOG.md:5,27-28,34` — depends on: nothing in-repo (probes are read-only: `ls`/`pgrep`/`stat`) — proof: simulated outage → alarm within 2 min; a night without an outage → 0 alarms + the 09:00 heartbeat; plus a mutation of a check that must fail.
- `[✓/verify]` `watchdog` key in `/api/diagnostics` + About-panel line — **appears done in `467b6a60`**, still listed as open in `PLAN.md:117-118` and `24_WATCHDOG.md:5` — proof: re-read the commit before scheduling.
- `[S, owner]` Install the timer: `WATCHDOG_WEBHOOK_URL` into the run-dir `.env`, then `sudo cp deploy/systemd/etlegacy-watchdog.* /etc/systemd/system/ && daemon-reload && enable --now` — `PLAN.md:113-115` — depends on: **owner** — proof: the r. 1 dry-run already showed 9/9 `ok` (disk 84.7 %, just under the 85 threshold) and `bot_streaks` `unknown`.
- `[S]` Fix the path in `~/slomix-server-logs/bin/pull_puran_console_log.sh` — **outside the repo, owner** — `PLAN.md:118`.
- `[S, owner]` **Restart the dev bot**: #923 (SSH monitor) merged at 18:02 on 2026-09-06, but the owner's 17:17 restart ran `ed0dfe22` — **not one line of #923 is running on dev** — `PLAN.md:438-441` — proof: `sudo systemctl restart etlegacy-bot.service` on dev, then confirm the streaks file appears.
...
- `[S]` Slice 3: show/hide sections on the session page (+ `Hidden` in `ui.tsx`) — `§9.3` — proof: the hidden section is **not in the DOM** (Playwright), un-hiding without reload, anonymous parity inventory unchanged.
...
- `[S]` Dependency policy: settle `requirements.txt` (runtime) vs `requirements-dev.txt` (test/lint) and make CI install dev deps consistently — `:69-72`.
...
- `[M]` **Security completion, still open from the original objective**: CSRF protection for state-changing endpoints; production CORS allowlist validation; explicit TLS reverse-proxy guidance in the deployment docs — `:108-111`.
...
- **`rounds/{}/awards` placement** — there is no per-round surface yet — `HANDOFF-next.md:104-105`.
...
- **Owner actions, not decisions** (queued): install the watchdog timer + `WATCHDOG_WEBHOOK_URL`; restart the dev bot so #923 actually runs; merge #955/#956/#958/#960; leave #912 to the sister session."}],"stop_reason":null,"stop_sequence":null,"stop_details":null,"usage":{"input_tokens":2,"cache_creation_input_tokens":4018,"cache_read_input_tokens":108083,"cache_creation":{"ephemeral_5m_input_tokens":4018,"ephemeral_1h_input_tokens":0},"output_tokens":3,"service_tier":"standard","inference_geo":"not_available"},"diagnostics":null},"apiBlockIndex":1,"requestId":"req_011CeooPyubMXDazjsKb1bBt","attributionAgent":"Explore","type":"assistant","uuid":"a277a80c-f982-437f-8b7c-dcc585bc8087","timestamp":"2026-09-07T09:00:19.863Z","effort":"high","userType":"external","entrypoint":"cli","cwd":"/home/samba/share/slomix_discord","sessionId":"dd0c663d-95e7-4730-bdf5-97d40a4ea7bf","version":"2.1.258","gitBranch":"HEAD","slug":"nifty-honking-scroll"}
