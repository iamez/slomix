...
- `prediction_event_key` is a deterministic hash of date, map, format, and sorted rosters, so a repeated voice event cannot create a duplicate. Resolution is idempotent and records the actual `gaming_session_id`.
