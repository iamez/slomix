...
For migrations 052–060, compare actual production objects (`pg_get_indexdef`, `pg_get_viewdef`, column types/defaults/nullability, constraints) against the committed SQL. Only on full match run the runner's `--mark` for exactly those nine files. Goal: `--validate` reports 0 pending / 0 failed / 0 checksum mismatch.
...
- `PREDICTION_SHADOW_ENABLED=true` generates and stores predictions; `PREDICTION_PUBLISH_ENABLED=false` suppresses every Discord embed. Public `/api/predictions/recent` filters `WHERE publish_state='published'` (initially: nothing).
...
- `prediction_event_key` is a deterministic hash of date, map, format, sorted rosters, **and an occurrence window** (the split time bucketed to the prediction cooldown). The occurrence component is essential: date+map+format+rosters alone would collapse a legitimate same-evening rematch of the same roster into the first prediction's key and silently drop its calibration sample. Because predictions fire at most once per cooldown window per split, a rematch after the cooldown lands in a new window (a new row), while a true repeated detection inside the same window still dedups. Resolution is idempotent and records the actual `gaming_session_id`.
...
- API additively gains `formula_version`, `quality`, `failed_sources`, `metric_weight_coverage`, `ranking_available`, per-player `missing_metrics`.
